...
async def get_queries_and_results_from_latest_checkpoint(checkpointer, thread_id: str):
    ...
    print__checkpointers_debug(
        f"279 - GET CHECKPOINT START: Getting queries and results from checkpoints for thread: {thread_id}"
    )
    try:
        config = {"configurable": {"thread_id": thread_id}}
        ...
        try:
            print__checkpointers_debug(
                "280 - ALIST METHOD: Using official AsyncPostgresSaver.alist() method"
            )
            ...
            print__checkpointers_debug(
                f"281 - ALIST ERROR: Error using alist(): {alist_error}"
            )

            # Fallback: use aget_tuple() to get the latest checkpoint only
            try:
                print__checkpointers_debug(
                    "282 - FALLBACK METHOD: Trying fallback method using aget_tuple()"
                )
                state_snapshot = await checkpointer.aget_tuple(config)
                if state_snapshot:
                    checkpoint_tuples = [state_snapshot]
                    print__checkpointers_debug(
                        "283 - FALLBACK SUCCESS: Using fallback method - got latest checkpoint only"
                    )
            except Exception as fallback_error:
                print__checkpointers_debug(
                    f"284 - FALLBACK ERROR: Fallback method also failed: {fallback_error}"
                )
                return []

        if not checkpoint_tuples:
            print__checkpointers_debug(
                f"285 - NO CHECKPOINTS: No checkpoints found for thread: {thread_id}"
            )
            return []

        print__checkpointers_debug(
            f"286 - CHECKPOINTS FOUND: Found {len(checkpoint_tuples)} checkpoints for thread"
        )
        ...
        print__checkpointers_debug(
            f"287 - QUERIES EXTRACTION: Extracting queries_and_results from {len(checkpoint_tuples)} checkpoints"
        )
        ...
        for checkpoint_index, checkpoint_tuple in enumerate(checkpoint_tuples):
            ...
            if isinstance(writes, dict) and "submit_final_answer" in writes:
                ...
                if (
                    ...
                    and "queries_and_results" in submit_data
                ):
                    ...
                    if queries_and_results:
                        ...
                        if isinstance(queries_and_results, list):
                            ...
                            print__checkpointers_debug(
                                f"288 - QUERIES FOUND: Step {step}: Found {len(queries_and_results)} queries and results"
                            )
                        else:
                            all_queries_and_results.append(queries_and_results)
                            print__checkpointers_debug(
                                f"289 - QUERIES FOUND: Step {step}: Found 1 query and result"
                            )

        print__checkpointers_debug(
            f"290 - GET CHECKPOINT SUCCESS: Found {len(all_queries_and_results)} total queries and results for thread: {thread_id}"
        )
        return all_queries_and_results

    except Exception as e:
        print__checkpointers_debug(
            f"291 - GET CHECKPOINT ERROR: Error getting queries and results from checkpoints: {e}"
        )
        return []
