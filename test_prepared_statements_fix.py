...
async def test_prepared_statements_fix():
    ...
    try:
        ...
        from my_agent.utils.postgres_checkpointer import get_connection_string, get_connection_kwargs
        
        connection_string = get_connection_string()
        connection_kwargs = get_connection_kwargs()
        
        if "prepare_threshold" not in connection_string:
            print("✅ prepare_threshold NOT in connection string (correct - should be in kwargs)")
        else:
            print("❌ prepare_threshold found in connection string (this should be fixed)")
            return False
            
        if "prepared_max" not in connection_string:
            print("✅ prepared_max NOT in connection string (correct - should be in kwargs)")
        else:
        ...
        if connection_kwargs.get("prepare_threshold") is None:
            ...
            print(f"❌ prepare_threshold not properly set in kwargs: {connection_kwargs.get('prepare_threshold')}")
            return False
        ...
        print(f"✅ aget() operation successful: {result is None} (expected for new thread)")
        ...
        async with await psycopg.AsyncConnection.connect(connection_string, **connection_kwargs) as conn:
            async with conn.cursor() as cur:
                ...
                if prepared_count == 0:
                    ...
                    print(f"⚠️ Found {prepared_count} prepared statements - they should be disabled")
                    print("   This might be from other connections, but the fix should prevent new ones")
