...
px_client = Client(warn_if_server_not_running=True)  # Updated initialization

# Convert to DataFrame
new_data_df = pd.DataFrame(agent_ground_truth.items(), columns=["question", "expected_answer"])
dataset_name = "agent_output_evaluation_langchain6"
...
# Define the async task function that will call our agent concurrently
import sys
import os
import asyncio
...
async def task(x):
    """Phoenix async task function: returns a JSON-serializable dict."""
    question = x["question"]
    # Run the blocking main() in a thread to allow concurrency
...
# Simplified evaluator: assumes expected is always a dict with "expected_answer"
import re
