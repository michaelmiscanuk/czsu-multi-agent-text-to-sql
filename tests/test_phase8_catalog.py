...
try:
    ...
    from api.routes.catalog import get_catalog, get_data_tables, get_data_table
...
def test_get_catalog_function():
    ...
    try:
        ...
        try:
            ...
            print_test_status(f"✅ Catalog query successful - found {result['total']} total items")
            print_test_status(f"✅ Returned {len(result['results'])} items on page {result['page']}")
            
        except Exception as catalog_error:
            # Database might not exist in test environment
            print_test_status(f"⚠️ Catalog database not available: {catalog_error}")
            print_test_status("ℹ️ This is expected in test environments without the catalog database")
...
def test_get_data_tables_function():
    ...
    try:
        ...
        try:
            ...
            # Check result structure
            assert isinstance(result, dict), "get_data_tables should return a dictionary"
            assert "tables" in result, "Result should have 'tables' field"
            assert isinstance(result["tables"], list), "Tables should be a list"
            
            print_test_status(f"✅ Data tables query successful - found {len(result['tables'])} tables")
            
        except Exception as tables_error:
            # Database might not exist in test environment
            print_test_status(f"⚠️ Data tables database not available: {tables_error}")
            print_test_status("ℹ️ This is expected in test environments without the data database")
...
def test_get_data_table_function():
    ...
    try:
        ...
        assert result["columns"] == [], "Should return empty columns when no table specified"
...
def test_catalog_authentication_dependencies():
    ...
    try:
        ...
        for route in router.routes:
            ...
            print_test_status(f"✅ Route {route.path} has {len(dependencies)} dependencies")
...
async def main():
    ...
    tests = [
        ...
        ("Catalog Authentication Dependencies", test_catalog_authentication_dependencies),
