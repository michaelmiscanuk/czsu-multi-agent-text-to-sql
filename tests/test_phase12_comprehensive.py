...
async def test_health_endpoints():
    ...
    try:
        async with httpx.AsyncClient(base_url=SERVER_BASE_URL, timeout=httpx.Timeout(REQUEST_TIMEOUT)) as client:
            ...
            assert response.status_code in [200, 503]  # 503 is acceptable for database issues
...
async def test_authentication_flow():
    ...
    try:
        ...
        async with httpx.AsyncClient(base_url=SERVER_BASE_URL, timeout=httpx.Timeout(REQUEST_TIMEOUT)) as client:
            ...
            print("✅ Authentication working - unauthenticated requests properly rejected")
...
async def test_analyze_endpoint():
    ...
    try:
        ...
        async with httpx.AsyncClient(base_url=SERVER_BASE_URL, timeout=httpx.Timeout(120)) as client:
...
async def test_chat_endpoints():
    ...
    try:
        ...
        async with httpx.AsyncClient(base_url=SERVER_BASE_URL, timeout=httpx.Timeout(REQUEST_TIMEOUT)) as client:
            ...
            response = await client.get(f"/chat/{test_thread_id}/messages", headers=headers)
            assert response.status_code in [200, 404]
            print("✅ /chat/{thread_id}/messages endpoint working")
            
            # Test chat sentiments endpoint
            response = await client.get(f"/chat/{test_thread_id}/sentiments", headers=headers)
            assert response.status_code in [200, 500]  # May error if thread doesn't exist
...
async def test_error_handling():
    ...
    try:
        async with httpx.AsyncClient(base_url=SERVER_BASE_URL, timeout=httpx.Timeout(REQUEST_TIMEOUT)) as client:
            ...
            assert response.status_code in [401, 422]  # 401 for missing auth, 422 for validation
...
async def main():
    ...
    if overall_success:
        print("🎉 ALL COMPREHENSIVE TESTS PASSED - Modular structure working correctly!")
    else:
