...
def test_phase4_models():
    ...
    assert (
        PaginatedChatThreadsResponse is not None
    ), "PaginatedChatThreadsResponse should be importable"
    ...
    valid_analyze = AnalyzeRequest(
        prompt="Test prompt for analysis", thread_id="test-thread-123"
    )
    ...
    try:
        ...
        assert (
            "string_too_short" in str(e)
            or "min_length" in str(e)
    ...
    valid_feedback = FeedbackRequest(
        run_id=test_uuid, feedback=1, comment="Great response!"
    )
    ...
    valid_feedback_comment_only = FeedbackRequest(
        run_id=test_uuid, comment="Just a comment"
    )
    ...
    empty_comment_feedback = FeedbackRequest(run_id=test_uuid, feedback=1, comment="")
    ...
    valid_sentiment = SentimentRequest(run_id=test_uuid, sentiment=True)
    ...
    valid_sentiment_neg = SentimentRequest(run_id=test_uuid, sentiment=False)
    ...
    valid_sentiment_null = SentimentRequest(run_id=test_uuid, sentiment=None)
    ...
    paginated_response = PaginatedChatThreadsResponse(
        threads=[thread_response], total_count=10, page=1, limit=5, has_more=True
    )
