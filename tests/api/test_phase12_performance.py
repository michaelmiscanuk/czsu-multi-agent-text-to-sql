...
async def test_memory_usage_patterns():
    ...
    try:
        ...
        print(
            f"📊 Baseline memory: {baseline_memory:.1f}MB"
            if baseline_memory
            else "📊 Could not measure baseline memory"
        )
        ...
        print(
            f"📊 Memory after requests: {final_memory:.1f}MB"
            if final_memory
            else "📊 Could not measure final memory"
        )
...
async def main():
    ...
    if summary["startup_time"]:
        startup_status = (
            ...
            else "⚠️ SLOW" if summary["startup_time"] < 30 else "❌ TOO SLOW"
        )
    ...
    if summary["memory_growth_mb"]:
        memory_status = (
            ...
            else "⚠️ HIGH" if summary["memory_growth_mb"] < 100 else "❌ EXCESSIVE"
        )
    ...
    if summary["avg_response_time"]:
        response_status = (
            ...
            else "⚠️ SLOW" if summary["avg_response_time"] < 3.0 else "❌ TOO SLOW"
        )
        print(
            f"⚡ Avg Response Time: {summary['avg_response_time']:.3f}s {response_status}"
        )
    ...
    if summary["concurrent_performance"]:
        ...
        concurrent_status = (
            "✅ GOOD"
            if success_rate >= 80
            else "⚠️ ISSUES" if success_rate >= 60 else "❌ POOR"
        )
