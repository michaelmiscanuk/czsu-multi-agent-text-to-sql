...
def create_test_token(audience: str = None, issuer: str = "test_issuer", email: str = "test@example.com", exp_minutes: int = 60):
    ...
    try:
        ...
        try:
            ...
            if e.status_code == 401:
                ...
                print_test_status(f"❌ Wrong status code for invalid token: {e.status_code}")
                return False
        ...
        if test_mode:
            ...
            try:
                ...
                print_test_status(f"✅ Test token verification successful: {result.get('email', 'Unknown')}")
            except HTTPException as e:
        ...
        try:
            ...
            if e.status_code == 401:
                ...
                print_test_status(f"❌ Wrong status code for expired token: {e.status_code}")
                return False
...
def test_get_current_user_function():
    ...
    try:
        ...
        try:
            ...
            if e.status_code == 401 and "Invalid Authorization header format" in e.detail:
        ...
        test_mode = os.getenv('USE_TEST_TOKENS', '0') == '1'
        if test_mode:
            print_test_status("🧪 TEST MODE ENABLED: Testing with valid authorization header")
            test_token = create_test_token()
            auth_header = f"Bearer {test_token}"
            
            try:
                result = get_current_user(auth_header)
                print_test_status(f"✅ Valid authorization header successful: {result.get('email', 'Unknown')}")
            except HTTPException as e:
                print_test_status(f"❌ Valid authorization header failed: {e.detail}")
                return False
        else:
            print_test_status("ℹ️ TEST MODE DISABLED: Skipping valid authorization header test")
...
def test_authentication_integration():
    ...
    try:
        ...
        if test_mode:
            print_test_status("🧪 TEST MODE ENABLED: Testing complete authentication flow")
            ...
            assert user_info.get('email') == test_email, f"Expected email {test_email}, got {user_info.get('email')}"
            assert user_info.get('iss') == 'test_issuer', f"Expected issuer 'test_issuer', got {user_info.get('iss')}"
            
            print_test_status(f"✅ Authentication integration successful for user: {user_info.get('email')}")
        else:
            print_test_status("ℹ️ TEST MODE DISABLED: Skipping authentication integration test")
...
def main():
    ...
    print_test_status(f"🔑 GOOGLE_CLIENT_ID: {os.getenv('GOOGLE_CLIENT_ID', 'NOT_SET')[:20]}...")
