...
async def test_modular_function_imports():
    ...
    total_tests += 1
    try:
        from my_agent.utils.postgres_checkpointer import get_healthy_checkpointer, create_thread_run_entry, initialize_checkpointer
    ...
    total_tests += 1
    try:
        from api.config.settings import GLOBAL_CHECKPOINTER, GC_MEMORY_THRESHOLD, start_time
