...
try:
    ...
    from api.routes.health import health_check, database_health_check, memory_health_check, rate_limit_health_check, prepared_statements_health_check
...
def test_phase8_health_imports():
    ...
    try:
        ...
        assert callable(database_health_check), "database_health_check should be callable"
        ...
        assert callable(rate_limit_health_check), "rate_limit_health_check should be callable"
        ...
        assert callable(prepared_statements_health_check), "prepared_statements_health_check should be callable"
        print_test_status("✅ prepared_statements_health_check function imported successfully")
...
async def test_health_check_function():
    ...
    try:
        ...
        assert "timestamp" in result_data, "Health check result should have 'timestamp' field"
        assert "memory" in result_data, "Health check result should have 'memory' field"
        assert "database" in result_data, "Health check result should have 'database' field"
        
        print_test_status(f"✅ Health check status: {result_data.get('status')}")
        print_test_status(f"✅ Memory usage: {result_data.get('memory', {}).get('rss_mb', 'Unknown')}MB")
        print_test_status(f"✅ Database status: {result_data.get('database', {}).get('healthy', 'Unknown')}")
...
async def test_database_health_check_function():
    ...
    try:
        ...
        assert "timestamp" in result_data, "Database health check result should have 'timestamp' field"
        assert "checkpointer_available" in result_data, "Database health check result should have 'checkpointer_available' field"
        
        print_test_status(f"✅ Checkpointer available: {result_data.get('checkpointer_available')}")
        print_test_status(f"✅ Checkpointer type: {result_data.get('checkpointer_type', 'Unknown')}")
...
async def test_memory_health_check_function():
    ...
    try:
        ...
        # Verify the result has expected structure
        assert "status" in result, "Memory health check result should have 'status' field"
        assert "memory_rss_mb" in result, "Memory health check result should have 'memory_rss_mb' field"
        assert "timestamp" in result, "Memory health check result should have 'timestamp' field"
...
async def test_rate_limit_health_check_function():
    ...
    try:
        ...
        # Verify the result has expected structure
        assert "status" in result, "Rate limit health check result should have 'status' field"
        assert "total_tracked_clients" in result, "Rate limit health check result should have 'total_tracked_clients' field"
        assert "timestamp" in result, "Rate limit health check result should have 'timestamp' field"
        
        print_test_status(f"✅ Rate limit status: {result.get('status')}")
        print_test_status(f"✅ Total tracked clients: {result.get('total_tracked_clients')}")
...
async def test_prepared_statements_health_check_function():
    ...
    try:
        ...
        assert "status" in result_data, "Prepared statements health check result should have 'status' field"
        assert "timestamp" in result_data, "Prepared statements health check result should have 'timestamp' field"
        
        print_test_status(f"✅ Prepared statements status: {result_data.get('status')}")
        print_test_status(f"✅ Checkpointer status: {result_data.get('checkpointer_status', 'Unknown')}")
        
        print_test_status("✅ prepared_statements_health_check function test PASSED")
        return True
        
    except Exception as e:
        print_test_status(f"❌ prepared_statements_health_check function test FAILED: {e}")
        print_test_status(f"❌ Full traceback:\n{traceback.format_exc()}")
        return False
...
def test_health_router_structure():
    ...
    try:
        ...
        expected_paths = ["/health", "/health/database", "/health/memory", "/health/rate-limits", "/health/prepared-statements"]
        
        for expected_path in expected_paths:
            assert expected_path in route_paths, f"Expected route {expected_path} not found in router"
...
async def main():
    ...
    tests = [
        ...
        ("Prepared Statements Health Check Function", test_prepared_statements_health_check_function),
    ]
