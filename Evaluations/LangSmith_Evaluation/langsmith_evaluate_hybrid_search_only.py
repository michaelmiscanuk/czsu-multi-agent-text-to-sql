...
EXPERIMENT_CONFIG = {
    ...
    "evaluators": ["selection_correct_hybrid", "selection_in_top_n_hybrid"],   # List of evaluator functions to use
}
...
async def selection_correct_hybrid(outputs: dict, reference_outputs: dict) -> bool:
    ...
    if hybrid_results and len(hybrid_results) > 0:
        ...
        if hasattr(first_doc, 'metadata') and first_doc.metadata:
            actual_selection = str(first_doc.metadata.get('selection', '')).strip().upper()

    result = (actual_selection == expected_selection)
    print(f"[Evaluator: correct_hybrid] return: {result} (actual: {actual_selection}, expected: {expected_selection})")
    return result
...
async def selection_in_top_n_hybrid(outputs: dict, reference_outputs: dict) -> bool:
    ...
    print(f"[Evaluator: in_top_n_hybrid] return: {found} (expected: {expected_selection}, candidates: {candidates[:10]})")  # Show first 10 candidates
    return found
...
def example_to_state(inputs: dict) -> dict:
    ...
    return {
        ...
        "rewritten_prompt": inputs['question'],  # For hybrid search, use the question directly
        "messages": [],
        "iteration": 0,
        "queries_and_results": [],
        "reflection_decision": "",
...
async def retry_hybrid_search(inputs, max_attempts=6, wait_seconds=10):
    ...
    while attempt < max_attempts:
        ...
        if hybrid_results:
            print(f"[retry_hybrid_search] Got {len(hybrid_results)} results for prompt: {prompt} on attempt {attempt}")
            return output
        else:
            print(f"[retry_hybrid_search] Empty hybrid search output for prompt: {prompt} (attempt {attempt}), waiting {wait_seconds}s before retry...")
            await asyncio.sleep(wait_seconds)
    
    print(f"[retry_hybrid_search] WARNING: No result for prompt: {prompt} after {max_attempts} attempts!")
    return output
