...
from my_agent.utils.nodes import retrieve_similar_selections_hybrid_search_node, rerank_node, relevant_selections_node
...
EXPERIMENT_CONFIG = {
    ...
    "evaluators": ["selection_correct", "selection_in_top_n"],   # List of evaluator functions to use
}
...
def example_to_state(inputs: dict) -> dict:
    ...
    return {
        ...
        "rewritten_prompt": inputs['question'],  # Use the question directly for the pipeline
        "messages": [],
        "iteration": 0,
        "queries_and_results": [],
        "reflection_decision": "",
...
async def retry_node_sequence(inputs, max_attempts=6, wait_seconds=10):
    ...
    while attempt < max_attempts:
        ...
        if not hybrid_results:
            print(f"[retry_node_sequence] Empty hybrid search output for prompt: {prompt} (attempt {attempt}), waiting {wait_seconds}s before retry...")
            await asyncio.sleep(wait_seconds)
            continue
        
        print(f"[retry_node_sequence] Hybrid search returned {len(hybrid_results)} results, proceeding to rerank...")
        ...
        if most_similar:
            print(f"[retry_node_sequence] Got {len(most_similar)} reranked results for prompt: {prompt} on attempt {attempt}")
            print(f"[retry_node_sequence] Top 3 results: {most_similar[:3]}")
            return final_output
        else:
            print(f"[retry_node_sequence] Empty rerank output for prompt: {prompt} (attempt {attempt}), waiting {wait_seconds}s before retry...")
            await asyncio.sleep(wait_seconds)
    
    print(f"[retry_node_sequence] WARNING: No result for prompt: {prompt} after {max_attempts} attempts!")
    return final_output
