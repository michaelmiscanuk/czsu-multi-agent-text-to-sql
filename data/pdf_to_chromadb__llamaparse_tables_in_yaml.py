...
CHROMA_DB_PATH = SCRIPT_DIR / "pdf_chromadb_llamaparse_v3"
...
def get_llamaparse_instructions() -> str:
    ...
    """
    return f"""
🚨 CRITICAL: OUTPUT TABLES AS HTML TABLES! 🚨
🚨 EXTRACT EVERY VALUE - NO MISSING DATA! 🚨


MOST IMPORTANT RULES:
- Output tables as HTML tables (<table>, <tr>, <td> format)
- Numbers without separators: "49621" not "49,621"
- Keep all original data intact - do not summarize or omit any values

=== TABLE PROCESSING ===

STEP 1: Identify tables and output them as HTML tables
- Convert all tables to HTML table format with proper <table>, <tr>, <td> tags
- Structure: <table><tr><td>header1</td><td>header2</td></tr><tr><td>value1</td><td>value2</td></tr></table>
- Preserve all numerical data exactly as shown
- Include all years, categories, and values present in the original table
- Do not convert to sentences - keep tabular structure with HTML

STEP 2: For complex multi-year tables
- Use HTML table structure with multiple rows and columns
...
- When tables span multiple pages, consolidate them into single complete tables
...
- Maintain proper table structure and formatting in the consolidated result
...
- Look and read where each data point actually falls, describe each curvature each bar, each pie slice etc.
...
🚨 OUTPUT TABLES IN YAML FORMAT - DO NOT USE CUSTOM SEPARATORS! 🚨
"""
...
def convert_html_table_to_yaml(html_table: str) -> str:
    """
    Convert HTML table to YAML format for better reranking.
    """
    try:
        # Parse HTML table
        ...
        if tbody:
            ...
            # Skip header row if already used
        ...
        for i, row in enumerate(rows):
            ...
            for j, value in enumerate(row):
                ...
                if j < len(headers):
                    ...
                    clean_header = (
                        re.sub(r"[^\w\s-]", "", header_name)
                        .strip()
                        .lower()
                        .replace(" ", "_")
                    )
                else:
                    clean_header = f"col_{j+1}"

                # Escape special characters and format for YAML
                clean_value = value.replace('"', '\\"').replace("\n", " ")
                yaml_output.append(f'    {clean_header}: "{clean_value}"')

        return "\n".join(yaml_output)

    except Exception as e:
        print__chromadb_debug(f"HTML to YAML conversion failed: {e}")
        return html_table  # Return original on error


def convert_html_tables_to_markdown(text):
    """Convert all HTML tables in text to markdown format."""
    import re

    def replace_table(match):
        html = match.group(0)
        return convert_html_table_to_markdown(html)
    ...
    try:
        ...
        try:
            ...
            parser = LlamaParse(
                ...
                page_separator="\n\n--- Page {page_number} ---\n\n",
                verbose=True,
                parse_mode="parse_document_with_llm",  # Changed from parse_page_with_lvm for multi-page table support
                ...
                output_tables_as_HTML=True,  # Explicitly output tables as HTML
            )
...
def custom_structural_chunking(
    text: str, source_files: set, page_numbers: list
) -> List[object]:
    """
    Custom structural chunking that preserves table structure without LLM dependency.
    Splits text by HTML tables, converts them to YAML, and other structural elements.
    Tables are converted from HTML to YAML format during chunking.

    Args:
        text: Combined markdown text to chunk
        source_files: Set of source files
        page_numbers: List of page numbers

    Returns:
        List of node-like objects with content and metadata
    """
    import re
    ...
    def create_node(content, node_type):
        """Helper function to create a node with proper content capture"""
        node = SimpleNamespace()
        node.content = content  # Store content directly
        node.node_type = node_type
        node.get_content = lambda: content  # Lambda captures content by value
        return node

    for line in lines:
        # Check if we're entering or leaving an HTML table
        if "<table" in line and ">" in line:
            # Save any accumulated content before the table
            if current_content and current_type != "table":
                content = "\n".join(current_content).strip()
                if content:
                    nodes.append(create_node(content, current_type))
                current_content = []
        ...
        elif in_table and "</table>" in line:
            # End of table
            table_content.append(line)
            # Convert HTML table to YAML and save
            html_table = "\n".join(table_content).strip()
            yaml_table = convert_html_table_to_yaml(html_table)
            if yaml_table:
                nodes.append(create_node(yaml_table, "table"))
            in_table = False
            table_content = []
            current_content = []
            current_type = "text"
        ...
        # Check for headers (markdown headers)
        if line.startswith("#"):
    ...
    # If we didn't find any structural elements, create a single node
    if not nodes:
        nodes = [create_node(text, "text")]

    print__chromadb_debug(f"Custom structural chunking: created {len(nodes)} nodes")
    for i, node in enumerate(nodes):
        content_preview = (
            node.get_content()[:100] + "..."
            if len(node.get_content()) > 100
            else node.get_content()
        )
        print__chromadb_debug(
            f"  Node {i}: {node.node_type} ({len(node.get_content())} chars) - {content_preview}"
        )
...
def process_parsed_text_to_chunks(
    ...
    # Use custom structural chunking that preserves table structure without LLM dependency
    # This implements the same logic as MarkdownElementNodeParser but without LLM requirements
    print__chromadb_debug("Using custom structural chunking (no LLM required)")
    ...
    # Custom structural chunking that preserves tables and headers
    nodes = custom_structural_chunking(document.text, source_files, page_numbers)
...
def format_table_for_reranking(text: str) -> str:
    """
    Format tabular data for better Cohere reranking performance.
    Converts HTML tables to YAML format for better understanding.
    """
    import re

    # Check if text contains YAML tables (already formatted)
    if "table_data:" in text and "row_" in text:
        # Tables are already in YAML format, return as-is
        return text

    # Check if text contains HTML tables that need conversion
    if "<table" in text and "</table>" in text:
        # Convert HTML tables to YAML
        return convert_html_table_to_yaml(text)

    # Fallback: Check if text contains markdown tables that need conversion
