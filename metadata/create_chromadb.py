...
def split_text_by_tokens(text: str, max_tokens: int = MAX_TOKENS) -> List[str]:
    ...
    for i in range(num_chunks):
        ...
        # If chunk is still too long, try to find a better split point
        while num_tokens_from_string(chunk) > max_tokens:
            # Try to find a period or newline to split on
            split_point = max(
                chunk.rfind('. '),
                chunk.rfind('\n'),
                chunk.rfind(' '),
                chunk_size // 2  # Fallback to middle if no good split point
            )
            if split_point == -1:
                split_point = chunk_size // 2
