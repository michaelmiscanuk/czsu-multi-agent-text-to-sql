...
async def test_postgres_checkpointer_import():
    ...
    try:
        ...
        try:
            from checkpointer.postgres_checkpointer import get_fallback_checkpointer
...
async def test_checkpointer_creation():
    ...
    try:
        ...
        checkpoint_config = await checkpointer.aput(
            config_dict, test_checkpoint, metadata, new_versions={}
        )
...
async def test_workflow_integration():
    ...
    try:
        ...
        if retrieved_state and retrieved_state.checkpoint:
            ...
            if (
                len(persisted_messages) == expected_messages
                and persisted_steps == expected_steps
            ):
    ...
    try:
        ...
        for i in range(rapid_operations):
            ...
            test_checkpoint = {
                ...
                "ts": time.time(),
                "id": str(uuid.uuid4()),
                "channel_values": {"operation": i, "message": f"Rapid operation {i}"},
            ...
            await rapid_checkpointer.aput(
                config, test_checkpoint, metadata, new_versions={}
            )
    ...
    try:
        ...
        try:
            ...
            if invalid_checkpointer is None:
                print(
                    f"   SUCCESS: Invalid connection properly handled (returned None)"
                )
            else:
                ...
                try:
                    ...
                    print(
                        f"   SUCCESS: Invalid connection failed during setup (expected)"
                    )

        except Exception as e:
            print(
                f"   SUCCESS: Invalid connection raised exception (expected): {type(e).__name__}"
            )
        ...
        try:
            ...
            await checkpointer.aput(
                config, malformed_checkpoint, metadata, new_versions={}
            )
            print(f"   WARNING: Malformed checkpoint was accepted")

        except Exception as e:
            print(
                f"   SUCCESS: Malformed checkpoint properly rejected: {type(e).__name__}"
            )
