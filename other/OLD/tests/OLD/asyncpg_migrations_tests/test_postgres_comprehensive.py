...
class PostgreSQLTestSuite:
    ...
    async def test_environment_setup(self):
        ...
        try:
            ...
            if not env_ok:
                print(
                    f"{TestColors.FAIL}[ERROR] Cannot continue without proper environment setup{TestColors.ENDC}"
                )
                return False
            ...
            self.log_test(
                "Basic Connection", basic_conn, "Direct asyncpg connection failed"
            )
    ...
    async def test_asyncpg_pool_operations(self):
        ...
        try:
            ...
            self.log_test(
                "Pool Creation", pool is not None, "Failed to create connection pool"
            )
            ...
            async with pool.acquire() as conn:
                ...
                self.log_test(
                    "Basic Pool Query", result == 1, f"Expected 1, got {result}"
                )
            ...
            async with pool.acquire() as conn:
                await conn.execute(
                    "CREATE TEMPORARY TABLE IF NOT EXISTS test_table (id SERIAL PRIMARY KEY, value TEXT)"
                )
                await conn.execute(
                    "INSERT INTO test_table (value) VALUES ($1), ($2)", "test1", "test2"
                )
                rows = await conn.fetch("SELECT * FROM test_table")
                self.log_test(
                    "Multiple Queries",
                    len(rows) >= 2,
                    f"Expected at least 2 rows, got {len(rows)}",
                )
            ...
            self.log_test(
                "AsyncPG Pool Operations", False, f"Unexpected exception: {str(e)}"
            )
            traceback.print_exc()
            return False
    ...
    async def test_users_threads_runs_table(self):
        ...
        try:
            ...
            self.log_test(
                "Data Retrieval (Single Thread)",
                found_thread,
                "Thread not found in results",
            )

            # Test thread count
            count = await get_user_chat_threads_count(test_email)
            self.log_test(
                "Thread Count", count >= 1, f"Expected at least 1, got {count}"
            )

            # Test sentiment update
            sentiment_result = await update_thread_run_sentiment(
                run_id, True, test_email
            )
            self.log_test(
                "Sentiment Update", sentiment_result, "Failed to update sentiment"
            )
            ...
            self.log_test(
                "Users Threads Runs Table", False, f"Unexpected exception: {str(e)}"
            )
            traceback.print_exc()
            return False
    ...
    async def test_langgraph_checkpointer(self):
        ...
        try:
            ...
            self.log_test(
                "Checkpointer Get (Empty)",
                result is None,
                f"Expected None for new thread, got {result}",
            )
            ...
            self.log_test(
                ...
                and hasattr(result_tuple, "checkpoint")
                and hasattr(result_tuple, "metadata"),
                "Failed to retrieve checkpoint tuple",
            )

            if result_tuple:
                metadata_ok = (
                    result_tuple.metadata is not None
                    and "test_metadata" in result_tuple.metadata
                )
                self.log_test(
                    "Checkpointer Metadata",
                    metadata_ok,
            ...
            self.log_test(
                "LangGraph Checkpointer", False, f"Unexpected exception: {str(e)}"
            )
            traceback.print_exc()
            return False
    ...
    async def test_conversation_message_extraction(self):
        ...
        try:
            ...
            await create_thread_run_entry(
                test_email, test_thread_id, "Test conversation extraction"
            )
            self.cleanup_items.append(("thread_entry", test_email, test_thread_id))
            ...
            self.log_test(
                "Message Extraction",
                len(messages) > 0,
                f"Expected messages, got {len(messages)}",
            )

            # Test queries extraction
            checkpoint = {
                "channel_values": {
                    "queries_and_results": [["SELECT * FROM test", "Test result"]]
                }
            }
            await checkpointer.aput(config, checkpoint, {}, {})

            queries = await get_queries_and_results_from_latest_checkpoint(
                checkpointer, test_thread_id
            )
            self.log_test(
                "Queries Extraction",
                len(queries) > 0,
                f"Expected queries, got {len(queries)}",
            )
        ...
        try:
            ...
            async def concurrent_asyncpg_task(task_id: int):
                ...
                async with pool.acquire() as conn:
                    ...
                    await conn.execute(
                        f"CREATE TEMPORARY TABLE {table_name} (id SERIAL PRIMARY KEY, value TEXT)"
                    )
            ...
            async def concurrent_checkpointer_task(task_id: int):
                ...
                checkpoint = {
                    "channel_values": {
                        "messages": [
                            {"content": f"Concurrent test {task_id}", "role": "user"}
                        ],
            ...
            asyncpg_results = await asyncio.gather(
                *asyncpg_tasks, return_exceptions=True
            )

            asyncpg_success = all(
                isinstance(r, int) and r == 5 for r in asyncpg_results
            )
            self.log_test(
                "Concurrent AsyncPG",
                asyncpg_success,
            ...
            checkpointer_results = await asyncio.gather(
                *checkpointer_tasks, return_exceptions=True
            )

            checkpointer_success = all(
                isinstance(r, bool) and r for r in checkpointer_results
            )
            self.log_test(
                "Concurrent Checkpointer",
                checkpointer_success,
            ...
            self.log_test(
                "Concurrent Operations", False, f"Unexpected exception: {str(e)}"
            )
            traceback.print_exc()
            return False
    ...
    async def test_error_handling_and_resilience(self):
        ...
        try:
            ...
            try:
                ...
                self.log_test(
                    "Invalid Query Handling", False, "Should have raised an exception"
                )
            except Exception:
                self.log_test(
                    "Invalid Query Handling", True, "Properly handled invalid query"
                )
            ...
            try:
                ...
                self.log_test(
                    "Checkpointer Error Handling",
                    True,
                    "Properly handled invalid checkpointer config",
                )
    ...
    async def test_performance_benchmarks(self):
        ...
        try:
            ...
            self.log_test(
                "AsyncPG Single Connection (100 queries)",
                True,
            ...
            self.log_test(
                "AsyncPG Concurrent (5x20 queries)",
                True,
            ...
            self.log_test(
                "Checkpointer Performance (10 gets)",
                True,
...
async def main():
    ...
    print(
        "Testing both AsyncPG (application) and LangGraph AsyncPostgresSaver (checkpointing)"
    )
    print("=" * 80)
