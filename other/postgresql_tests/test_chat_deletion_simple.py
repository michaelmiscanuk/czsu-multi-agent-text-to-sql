    ...
    try:
        ...
        async with checkpointer.pool.connection() as conn:
            for table in tables:
                try:
                    ...
                    print(f"   📊 Records for {test_thread_id}: {initial_counts[table]}")
            ...
            async with checkpointer.pool.connection() as conn:
                ...
                for table in tables:
                    try:
                        ...
                        print(f"   🗑️ Deleted {deleted_counts[table]} records from {table}")
        ...
        try:
            async with checkpointer.pool.connection() as conn:
                ...
                total_remaining = sum(count for count in final_counts.values() if isinstance(count, int))
