...
class PostgreSQLStressTest:
    ...
    def __init__(self):
        self.results = []
        self.test_emails = [f"stress_user_{i}_{uuid.uuid4().hex[:6]}@test.com" for i in range(10)]
        self.cleanup_threads = []
        
    def log_result(self, test_name: str, success: bool, details: Dict[str, Any]):
        """Log stress test result"""
        status = f"{StressTestColors.OKGREEN}✅ PASS{StressTestColors.ENDC}" if success else f"{StressTestColors.FAIL}❌ FAIL{StressTestColors.ENDC}"
    ...
    async def test_high_concurrency_asyncpg(self, concurrent_tasks: int = 50, operations_per_task: int = 20):
            ...
            try:
                ...
                async with pool.acquire() as conn:
                    for i in range(operations_per_task):
                        op_start = time.time()
                        result = await conn.fetchval("SELECT $1 + $2 + $3", task_id, i, random.randint(1, 100))
                        op_end = time.time()
        ...
        try:
            ...
            for i in range(max_size + 5):  # Try to exceed pool size
                ...
                try:
                    ...
                    print(f"    Connection {i+1}: Timeout (expected for pool exhaustion)")
                    break
                except Exception as e:
            ...
            details = {
                ...
                "avg_acquisition_time": statistics.mean(acquisition_times) if acquisition_times else 0,
                "max_acquisition_time": max(acquisition_times) if acquisition_times else 0,
    ...
    async def test_checkpointer_stress(self, concurrent_threads: int = 20, operations_per_thread: int = 10):
        ...
        async def checkpointer_task(task_id: int):
            ...
            try:
                ...
                for i in range(operations_per_thread):
                    op_start = time.time()
                    config = {"configurable": {"thread_id": f"stress_test_{task_id}_{i}"}}
    ...
    async def test_mixed_workload_stress(self, duration_seconds: int = 30):
        ...
        async def checkpointer_worker():
            ...
            while time.time() < end_time:
                try:
                    config = {"configurable": {"thread_id": f"mixed_test_{random.randint(1, 100)}"}}
                    await checkpointer.aget(config)
                    checkpointer_ops += 1
                except Exception:
        ...
        async def data_worker():
            ...
            while time.time() < end_time:
                try:
                    ...
                    await create_thread_run_entry(email, thread_id, f"Test prompt {random.randint(1, 1000)}")
                    asyncpg_ops += 1
                except Exception:
                    asyncpg_errors += 1
        ...
        details = {
            ...
            "total_throughput": total_ops / actual_duration if actual_duration > 0 else 0,
    ...
    async def test_memory_usage_under_load(self, operations: int = 1000):
        ...
        try:
            ...
            for i in range(operations):
                ...
                if i % 100 == 0:
                    ...
                    if memory_growth > 100:
                        print(f"    Warning: Memory growth {memory_growth:.1f}MB at operation {i}")
            ...
            details = {
                ...
                "memory_per_operation_kb": (memory_growth * 1024) / operations if operations > 0 else 0,
                "duration": duration,
            ...
            for email in self.test_emails:
                try:
                    ...
                    for thread in threads:
                        ...
                        from my_agent.utils.postgres_checkpointer import delete_user_thread_entries
    ...
    def print_summary(self):
        ...
        print(f"{StressTestColors.OKGREEN}Passed: {passed_tests}{StressTestColors.ENDC}")
    ...
    try:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                print(f"{StressTestColors.FAIL}Test {i+1} crashed: {result}{StressTestColors.ENDC}")
                traceback.print_exc()
