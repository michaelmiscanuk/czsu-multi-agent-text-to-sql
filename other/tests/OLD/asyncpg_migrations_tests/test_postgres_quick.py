...
async def main():
    ...
    try:
        ...
        test.test("Environment Variables", env_ok, "Missing required environment variables")
        ...
        try:
            run_id = await create_thread_run_entry(test_email, test_thread_id, "Quick test prompt")
            test.test("Data Insertion", run_id is not None, "Failed to create thread run entry")
        ...
        try:
            ...
            test.test("Checkpointer Creation", checkpointer is not None, "Failed to create checkpointer")
            
            if checkpointer:
                config = {"configurable": {"thread_id": f"quick_test_{uuid.uuid4().hex[:8]}"}}
        ...
        try:
            ...
            test.test("Concurrent Operations", success, "Some concurrent operations failed")
        except Exception as e:
