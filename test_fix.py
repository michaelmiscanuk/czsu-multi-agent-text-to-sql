...
async def test_simplified_postgres():
    ...
    try:
        ...
        if manager:
            print(f"✅ Checkpointer manager created successfully: {type(manager).__name__}")
        else:
        ...
        async with manager as checkpointer:
            if checkpointer:
                print(f"✅ Checkpointer created successfully: {type(checkpointer).__name__}")
            else:
            ...
            try:
                ...
                print(f"⚠️ Conversation loading test error (expected for new thread): {e}")
...
async def test_api_compatibility():
    ...
    try:
        ...
        print(f"Testing API-style pagination: page={page}, limit={limit}, offset={offset}")
        ...
        print(f"  - Page: {page}/{((total_count - 1) // limit) + 1 if total_count > 0 else 1}")
        ...
        print(f"✅ Successfully converted {len(response_threads)} threads to API format")
